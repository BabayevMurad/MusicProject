...
function MusicPage() {
  ...
  useEffect(() => {
    const fetchMusics = async () => {
      const res = await axios.get("https://localhost:7243/music/list");
      setMusics(res.data);
    };
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {musics.length === 0 ? (
        <div className="text-center text-gray-500">
          Yüklənir və ya siyahı boşdur...
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2">
          {musics.map((music) => (
            <div key={music.id} className="bg-white shadow-md rounded-xl p-4 border">
              ...
              {showDropdown && selectedMusicId === music.id && (
                <div className="mt-2 border p-2 rounded shadow bg-white">
